...
*/
function bosa_university_slickNavHeight (){
...
*/
function bosa_university_wpMenuAccordion( selector ){
...
function bosa_university_fixed_header(){
  $notificationHight = $( '.notification-bar' ).height();
  $logo_selector = document.getElementById( 'headerLogo' );
  var width = $( window ).width();

  if ( $logo_selector && BOSAUNIVERSITY.fixed_nav && BOSAUNIVERSITY.fixed_header_logo ) { 
    if ( $mastheadHeight < $(window).scrollTop()){
      if ( BOSAUNIVERSITY.separate_logo == '' ){
        if ( BOSAUNIVERSITY.the_custom_logo !== '' ){
          $logo_selector.src = BOSAUNIVERSITY.the_custom_logo;
        }
      }else{
        $( '.site-header .site-branding img' ).css( 'display' , 'block' );
        if( !BOSAUNIVERSITY.mobile_fixed_nav_off || width >= 782 ){
          $logo_selector.src = BOSAUNIVERSITY.separate_logo;
        }
      }
    }else{
      if ( BOSAUNIVERSITY.header_two_logo !== '' && ( BOSAUNIVERSITY.is_front_page || BOSAUNIVERSITY.overlay_post || BOSAUNIVERSITY.overlay_page ) && BOSAUNIVERSITY.is_header_two ){
         $logo_selector.src = BOSAUNIVERSITY.header_two_logo;
      }else if ( BOSAUNIVERSITY.the_custom_logo !== '' ) {
          $logo_selector.src = BOSAUNIVERSITY.the_custom_logo;
      }else if ( BOSAUNIVERSITY.separate_logo !== '' ){
        $( '.site-header .site-branding img' ).css( 'display' , 'none' );
      }
    }
  }
  if ( $mastheadHeight > $( window ).scrollTop() || $( window ).scrollTop() == 0 ) {
      if ( BOSAUNIVERSITY.fixed_nav && $( '#masthead.site-header' ).hasClass( 'sticky-header' )){
          $( '#masthead.site-header' ).removeClass( 'sticky-header' );
          // Fixed header in admin bar
          if( BOSAUNIVERSITY.is_admin_bar_showing && width >= 782 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
          if( BOSAUNIVERSITY.is_admin_bar_showing && width <= 781 ){
            $( '.fixed-header' ).css( 'marginTop', 0 );
          }
      }
  }else if ( BOSAUNIVERSITY.fixed_nav && !$( '#masthead.site-header' ).hasClass( 'sticky-header' )){
    if( !BOSAUNIVERSITY.mobile_fixed_nav_off || width >= 782 ){
      ...
      if( BOSAUNIVERSITY.is_admin_bar_showing && width <= 600 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
      }
      if( BOSAUNIVERSITY.mobile_fixed_nav_off && width <= 781 ){
        $( '.fixed-header' ).css( 'marginTop', 0 );
    }
  }
}
...
*/
function bosa_university_header_two_postion() {
  var width = $( window ).width();

  if ( BOSAUNIVERSITY.is_header_two ) {
    if( BOSAUNIVERSITY.is_admin_bar_showing && width >= 782 ){
...
jQuery( document ).ready( function() {
  bosa_university_slickNavHeight();
  bosa_university_wpMenuAccordion( '#offcanvas-menu' );
  bosa_university_header_two_postion();
  ...
  */
  $( '.header-image-slider' ).slick({
      ...
      fade: BOSAUNIVERSITY.header_image_slider.fade,
      speed: parseInt( BOSAUNIVERSITY.header_image_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSAUNIVERSITY.header_image_slider.autoplay,
      autoplaySpeed: BOSAUNIVERSITY.header_image_slider.autoplaySpeed,
      infinite: true,
  ...
  */
  $( '#primary-menu' ).slicknav({
      ...
      label: BOSAUNIVERSITY.responsive_header_menu_text,
  ...
  */
  $( '.main-slider' ).slick({
      ...
      arrows: true,
      adaptiveHeight: false,
      fade: BOSAUNIVERSITY.main_slider.fade,
      speed: parseInt( BOSAUNIVERSITY.main_slider.fadeControl ),
      cssEase: 'linear',
      autoplay: BOSAUNIVERSITY.main_slider.autoplay,
      autoplaySpeed: BOSAUNIVERSITY.main_slider.autoplaySpeed,
      infinite: true,
  ...
  $( '.highlight-post-slider' ).slick({
      arrows: true,
      dots: true,
      slidesToShow: BOSAUNIVERSITY.home_highlight_posts.slidesToShow,
      slidesToScroll: 1,
      adaptiveHeight: false,
      autoplay: BOSAUNIVERSITY.home_highlight_posts.autoplay,
      autoplaySpeed: BOSAUNIVERSITY.home_highlight_posts.autoplaySpeed,
      infinite: false,
      rows: 0,
  ...
  * Sticky sidebar
  */
  if( BOSAUNIVERSITY.sticky_sidebar ){
...
jQuery( window ).on( 'load', function(){
  ...
  * Back to top
  */
  if( BOSAUNIVERSITY.enable_scroll_top == true && $(window).scrollTop() > 200 ){
...
jQuery( window ).on( 'scroll', function(){
  ...
  * Back to top
  */
  if( BOSAUNIVERSITY.disable_scroll_top == false && $(window).scrollTop() > 200 ){
    $( '#back-to-top' ).fadeIn( 200 );
  } else{
    $( '#back-to-top' ).fadeOut( 200 );
  }
